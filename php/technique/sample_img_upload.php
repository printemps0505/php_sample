...
// アップロード画像ファイルの保存
if ($_SERVER['REQUEST_METHOD'] === 'POST') {//フォームより何かが送られてきたかどうか
  if (is_uploaded_file($_FILES['new_img']['tmp_name']) === TRUE) {// HTTP POST でファイルがアップロードされたかどうかチェック
    ...
    if ($extension === 'jpg' || $extension === 'jpeg') {    // 指定の拡張子であるかどうかチェック
...
try {
  ...
  // エラーがなければ、アップロードした新しい画像ファイル名を保存
    ...
    try {
      // SQL文を作成
      $sql = 'INSERT INTO test_upload_img
                (img_file_name)
                VALUES
                (:img_file_name)';//名前付きプレースフォルダを利用
      // SQL文を実行する準備
      $stmt = $dbh->prepare($sql);
      // SQL文のプレースホルダに値をバインド
      $stmt->bindValue(1, $new_img_filename,    PDO::PARAM_STR);//画像だけど文字列として扱っている
       // SQLを実行
      $stmt->execute();
